...
def build_deck(slug: str, name: str | None):
    ...
    # Agencies with the full tenant treatment (real captures + real tool runs,
    # data under treatment-data/<slug>/, generated by agency_treatment.py).
    TREATED = {"uncommon": "Uncommon", "lego": "LEGO",
               "alt-shift": "Alt/Shift", "alien-baby": "Alien Baby"}
    if slug in TREATED and not (HERE / "treatment-data" / slug).exists():
        print(f"  NOTE [{slug}]: treatment-data missing, deck still carries BMF tenant assets")
    elif slug in TREATED:
        import json as _json
        import agency_treatment
        html = agency_treatment.rework(html, slug, name)
        _counts = _json.loads((HERE / "treatment-data" / slug / "counts.json").read_text())
        _shots = {
            f"{name} co-creation onboarding, 100% ready, {_counts['curated']} phantoms": HERE / "assets" / f"{slug}-onboarding.png",
            "The onboarding interview in the app": HERE / "assets" / f"{slug}-interview.png",
            f"The {name} brain, {_counts['curated']} curated phantoms": HERE / "assets" / f"{slug}-brain.png",
            "Culture Scan running on ALDI in the app": HERE / "assets" / f"{slug}-culture-scan.png",
            "Synthetic Panel running the ALDI idea in the app": HERE / "assets" / f"{slug}-synthetic-panel.png",
            "Brief Sharpener scoring a weak health-fund brief in the app": HERE / "assets" / f"{slug}-brief-sharpener.png",
            "Tools hub, the agency's brain pointed at a specific job": HERE / "assets" / f"{slug}-tools.png",
            "The ALDI film on the Colleague pipeline canvas": HERE / "assets" / "town-square-pipeline-canvas.png",
        }
        for alt, image_path in _shots.items():
            image_b64 = base64.b64encode(image_path.read_bytes()).decode()
            html, replacements_made = re.subn(
                rf'(<img src=")data:image/[^"]+(" alt="{re.escape(alt)}")',
                lambda match: f'{match.group(1)}data:image/png;base64,{image_b64}{match.group(2)}',
            ...
            assert replacements_made == 1, f"{slug} screenshot not found: {alt}"
    if missing:
